...
export function createDecks(): {
  ...
  const fireFighting = makeCard('tucker_bag', 'Fire Fighting Equipment', 'Prevent bushfire damage');
  ...
  const studRam: Card[] = Array.from({ length: 5 }, () => makeCard('stud_ram', 'Stud Ram', 'Breed extra sheep'));

  return { tuckerBag, stockSale, studRam };
}

export function drawCard<T extends Card>(deck: T[], count = 1): { card: T | null; deck: T[] } {
