...
describe('resolveTuckerBag', () => {
  ...
  it('Flood adds $200', () => {
    const state = createInitialGameState([{ displayName: 'P', isAI: false }]);
    state.decks.tuckerBag = [{ id: '1', type: 'tucker_bag', title: 'Flood', effect: 'Gain relief' }];
  ...
  it('Drought reduces unirrigated paddock sheep by 2', () => {
    const state = createInitialGameState([{ displayName: 'P', isAI: false }]);
    state.decks.tuckerBag = [{ id: '1', type: 'tucker_bag', title: 'Drought', effect: 'Lose sheep' }];
    state.board.stations[0].paddocks[0].sheepCount = 5;
...
describe('resolveStockSale', () => {
  it('Wool Price Up increases wool price', () => {
    const state = createInitialGameState([{ displayName: 'P', isAI: false }]);
    state.decks.stockSale = [{ id: '1', type: 'stock_sale', title: 'Wool Price Up', effect: '+$2' }];
  ...
  it('Wool Price Down decreases wool price', () => {
    const state = createInitialGameState([{ displayName: 'P', isAI: false }]);
    state.decks.stockSale = [{ id: '1', type: 'stock_sale', title: 'Wool Price Down', effect: '-$2' }];
  ...
  it('Sheep Price Up increases sheep price', () => {
    const state = createInitialGameState([{ displayName: 'P', isAI: false }]);
    state.decks.stockSale = [{ id: '1', type: 'stock_sale', title: 'Sheep Price Up', effect: '+$1' }];
...
describe('resolveStudRam', () => {
  it('adds sheep based on total flock', () => {
    const state = createInitialGameState([{ displayName: 'P', isAI: false }]);
    state.decks.studRam = [{ id: '1', type: 'stud_ram', title: 'Stud Ram', effect: 'Breed' }];
    ...
    const totalSheep = next.board.stations[0].paddocks.reduce((s, p) => s + p.sheepCount, 0);
