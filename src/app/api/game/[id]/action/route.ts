...
import { resolveBuyImprovement, resolvePlaceImprovement, resolveIrrigate } from '@/lib/game/improvements';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (action === 'roll') {
      if (state.phase !== 'roll') {
        return NextResponse.json({ error: 'Cannot roll in current phase' }, { status: 400 });
      }
    ...
    if (action === 'buy_improvement' && payload?.tileType) {
      if (state.phase !== 'action' && state.phase !== 'station') {
        return NextResponse.json({ error: 'Cannot buy in current phase' }, { status: 400 });
      }
      if (currentPlayer.isAI) {
        return NextResponse.json({ error: 'AI turn' }, { status: 400 });
      }
    ...
    if (action === 'place_improvement' && payload?.tileType !== undefined && payload?.paddockIndex !== undefined) {
      if (state.phase !== 'action' && state.phase !== 'station') {
        return NextResponse.json({ error: 'Cannot place in current phase' }, { status: 400 });
      }
      if (currentPlayer.isAI) {
        return NextResponse.json({ error: 'AI turn' }, { status: 400 });
      }
      newState = resolvePlaceImprovement(state, state.currentPlayerIndex, payload.paddockIndex, payload.tileType);
    }

    if (action === 'irrigate' && payload?.paddockIndex !== undefined) {
      if (state.phase !== 'action' && state.phase !== 'station') {
        return NextResponse.json({ error: 'Cannot irrigate in current phase' }, { status: 400 });
      }
      if (currentPlayer.isAI) {
        return NextResponse.json({ error: 'AI turn' }, { status: 400 });
      }
    ...
    if (action === 'end_turn') {
      if (state.phase !== 'action' && state.phase !== 'station') {
        return NextResponse.json({ error: 'Cannot end turn in current phase' }, { status: 400 });
      }
      if (currentPlayer.isAI) {
        return NextResponse.json({ error: 'AI turn' }, { status: 400 });
      }
