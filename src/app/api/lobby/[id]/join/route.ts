...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (lobby.status !== 'WAITING') {
      return NextResponse.json({ error: 'Lobby has already started' }, { status: 400 });
    }
    ...
    const existing = lobby.players.find((p) => p.displayName.toLowerCase() === displayName.toLowerCase());
    if (existing) {
      return NextResponse.json({ error: 'A player with that name is already in the lobby' }, { status: 400 });
    }
