...
test.describe("Home page", () => {
  test("loads with title", async ({ page }) => {
    await page.goto("/");
    await expect(
      page.getByRole("heading", { name: /Sheep Tycoon/i }),
    ).toBeVisible();
  });

  test("Play vs Computer and Create/Join Lobby links are visible", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(
      page.getByRole("link", { name: /Play vs Computer/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: /Create \/ Join Lobby/i }),
    ).toBeVisible();
  });
  ...
  test("How to Play section is present", async ({ page }) => {
    await page.goto("/");
    await expect(
      page.getByRole("heading", { name: /How to Play/i }),
    ).toBeVisible();
  });
